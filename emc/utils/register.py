...
def apply_affine(moving, static, transform_affine, invert=False):
    ...
    affine_map = AffineMap(
        transform_affine, static.shape, static.affine, moving.shape,
        moving.affine
    )
    if invert is True:
...
def c_of_mass(
    moving, static, static_affine, moving_affine, reg, starting_affine,
        params0=None
):
    transform = transform_centers_of_mass(static, static_affine, moving,
                                          moving_affine)
