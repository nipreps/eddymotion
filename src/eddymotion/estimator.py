...
class EddyMotionEstimator:
    ...
    def fit(
        ...
        **kwargs,
        ...
        level : :obj:`int`
            Level of registration, selects the registration parameters from ANTs'
            pre-wired settings files. Registration is more precise (and slower) as
            level is higher.
        ...
        # Select the family of settings for the config file
        reg_target_type = (
            "dwi"
        ...
        if subsampling_factors:
            ...
            data = decimate(
                dwdata.dataobj,
                factor=subsampling_factors,
                smooth=smoothing_width if smoothing_width is not None else True
            )
        elif smoothing_width:
            from eddymotion.data.filtering import gaussian_filter

            data = gaussian_filter(data.dataobj, vox_width=smoothing_width)
            ...
            with tqdm(total=len(index_order), unit="dwi") as pbar:
                ...
                for i in index_order:
                    ...
                    registration = Registration(
                        ...
                        from_file=pkg_fn(
                            "eddymotion",
                            f"config/dwi-to-{reg_target_type}_level{level}.json",
                        ),
                    ...
                    if dwdata.em_affines and dwdata.em_affines[i] is not None:
                        mat_file = tmpdir / f"init_{level}_{i:05d}.mat"
